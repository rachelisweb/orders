-- Include the customer name in server-side Excel import failures so the
-- administrator can identify and correct the relevant source rows.
...
as $$
declare
  v_order jsonb; v_items jsonb; v_name text; v_customer_id uuid;
  v_result jsonb; v_results jsonb := '[]'::jsonb;
  v_count integer := 0; v_units integer := 0;
begin
  ...
  for v_order in select value from jsonb_array_elements(p_orders) loop
    ...
    if v_items is null or jsonb_typeof(v_items) <> 'array' or jsonb_array_length(v_items) = 0 then
      raise exception 'לקוח %: ההזמנה ריקה', v_name;
    end if;

    select c.id into v_customer_id from public.customers c
     where c.is_active and (lower(btrim(coalesce(c.business_name, ''))) = lower(v_name)
       or lower(btrim(coalesce(c.name, ''))) = lower(v_name))
     order by (lower(btrim(coalesce(c.business_name, ''))) = lower(v_name)) desc,
       (select count(*) from public.orders o where o.customer_id = c.id) desc, c.created_at asc limit 1;

    begin
      v_result := public.admin_create_order(v_customer_id,
        case when v_customer_id is null then jsonb_build_object('name', v_name, 'business_name', v_name) else null end,
        'נוצר מייבוא Excel', v_items);
    exception when others then
      raise exception 'לקוח %: %', v_name, sqlerrm;
    end;
