-- Keep customer contact snapshots in every order in sync with the customer card.
-- Pricing still changes only for uninvoiced, non-archived operational orders.
...
begin
  ...
  -- These fields are snapshots on the order, so explicitly synchronize them
  -- for pending, ready, shipped, archived, future, and cancelled orders alike.
  ...
  return jsonb_build_object(
    'ok', true,
    'synced_orders', v_synced,
    'repriced_orders', v_repriced
  );
end $$;
